...
# This class holds the context of the installer, meaning all of the target vars and paths that this instance is using.
...
class Context:
    ...
    def __init__(self) -> None:
        ...
        # Parsed from the command line args, if set, the addon install should be in update mode.
        self.IsUpdateMode:bool = False

        # Parsed from the command line args, if set, the addon install should be in uninstall mode.
    ...
    def LoadFromArgString(argString:str) -> 'Context':
        ...
        try:
            ...
            context = Context()
            context.RepoRootFolder = argObj["HA_REPO_DIR"]
            context.VirtualEnvPath = argObj["HA_ENV"]
    ...
    def Validate(self, generation:int=1) -> None:
        ...
        if generation >= 2:
            self._ValidatePathAndExists(self.AddonFolder, "Addon Folder Missing")
            self._ValidateString(self.InstanceId, "Instance Id missing.")
            self.AddonFolder = self.AddonFolder.strip()
            self.InstanceId = self.InstanceId.strip()

        if generation >= 3:
            self._ValidatePathAndExists(self.LogFolder, "Log folder is invalid.")
            self._ValidatePathAndExists(self.LocalDataFolder, "Local data folder is invalid.")
            self._ValidateString(self.ServiceFilePath, "Service file path missing.")
            self._ValidateString(self.ServiceName, "service name missing.")

        if generation >= 4:
            # The addon ID can be None, this means it didn't exist before we installed the service.
            pass
